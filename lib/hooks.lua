...
--Hook: Force Charm Tag on Ante 1 Small Blind when playing Suit Focus deck
local nexttag = get_next_tag_key
function get_next_tag_key(...)
    if G.GAME and (G.GAME.selected_back_key.key == "b_minty_suitfocus" or G.GAME.selected_sleeve == "sleeve_minty_suitfocussleeve") and not G.GAME.minty_suit_focus_first_tag_generated then
        G.GAME.minty_suit_focus_first_tag_generated = true
...
--Hook: force the pack generated by that first charm tag to include Lovers and Tower
local creat = create_card
function create_card(_type, area, legendary, _rarity, skip_materialize, soulable, forced_key, key_append)
    if key_append == "ar1" and not forced_key and (G.GAME.selected_back_key.key == "b_minty_suitfocus" or G.GAME.selected_sleeve == "sleeve_minty_suitfocussleeve") and (G.GAME.minty_suit_focus_force_first_pack or 0) < 5 then
        G.GAME.minty_suit_focus_force_first_pack = (G.GAME.minty_suit_focus_force_first_pack or 0) + 1
        if G.GAME.minty_suit_focus_force_first_pack == 1 then
            forced_key = "c_lovers"
        elseif G.GAME.minty_suit_focus_force_first_pack == 5 then
            forced_key = "c_tower"
        end
    end
